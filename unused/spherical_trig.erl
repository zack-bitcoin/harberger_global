...
cross(#point{x = X1, y = Y1, z = Z1},
      #point{x = X2, y = Y2, z = Z2}) ->
    %notice we do NOT simplify this point by removing the greatest common denominator. We need to preserve all the signs because they tell us which side of the world we are on.
    #point{x = (Y1 * Z2) - (Z1 * Y2),
           y = (Z1 * X2) - (X1 * Z2),
           z = (X1 * Y2) - (Y1 * X2)}.
